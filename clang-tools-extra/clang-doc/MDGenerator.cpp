...
///
/// \file
/// This file contains the implementation of the MDGenerator, which
...
namespace {
struct MDGenerator : public MustacheGenerator {
...
Error MDGenerator::setupTemplateFiles(const ClangDocContext &CDCtx) {
...
Error MDGenerator::generateDocumentation(
...
Error MDGenerator::generateDocForJSON(json::Value &JSON, raw_fd_ostream &OS,
                                      const ClangDocContext &CDCtx,
...
Error MDGenerator::generateDocForInfo(Info *I, raw_ostream &OS,
                                      const ClangDocContext &CDCtx) {
  return Error::success();
}
