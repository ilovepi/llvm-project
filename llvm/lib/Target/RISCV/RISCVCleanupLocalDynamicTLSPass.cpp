//===-- RISCVCleanupLocalDynamicTLSPass.cpp ---------------------*- C++ -*-=//
//
...
// 2. The variable's offset from _TLS_MODULE_BASE_ is calculated.
// 3. These two are added, together with TO, to obtain the variable's
//    true address.
//
...
#define TLSCLEANUP_PASS_NAME "RISCV Local Dynamic TLS Access Clean-up"

namespace {
struct RISCVLDTLSCleanup : public MachineFunctionPass {
  static char ID;
  RISCVLDTLSCleanup() : MachineFunctionPass(ID) {
    initializeRISCVLDTLSCleanupPass(*PassRegistry::getPassRegistry());
  }
  ...
  MachineInstr *replaceTLSBaseAddrCall(MachineInstr &I,
    ...
    // Insert a Copy from TLSBaseAddrReg to X10, which is where the rest of the
  ...
  MachineInstr *setRegister(MachineInstr &I, unsigned *TLSBaseAddrReg) {
    ...
    *TLSBaseAddrReg = RegInfo.createVirtualRegister(&RISCV::GPRRegClass);
    MachineInstr *Copy =
...
INITIALIZE_PASS(RISCVLDTLSCleanup, "riscv-local-dynamic-tls-cleanup",
                TLSCLEANUP_PASS_NAME, false, false)

char RISCVLDTLSCleanup::ID = 0;
FunctionPass *llvm::createRISCVCleanupLocalDynamicTLSPass() {
  return new RISCVLDTLSCleanup();
}
